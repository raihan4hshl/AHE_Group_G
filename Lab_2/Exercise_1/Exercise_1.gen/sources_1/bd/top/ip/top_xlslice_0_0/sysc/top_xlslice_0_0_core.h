...
#ifndef _top_xlslice_0_0_core_h_
#define _top_xlslice_0_0_core_h_

#include <systemc.h>
#include "properties.h"


class top_xlslice_0_0_core : public sc_module 
{
    public: 
       top_xlslice_0_0_core (sc_core::sc_module_name nm, const xsc::common_cpp::properties& props)
            : sc_module(nm)
              , Din    ( "Din" )
              , Dout   ( "Dout" )
    {
        SC_HAS_PROCESS(top_xlslice_0_0_core);
        SC_METHOD(get_slice_value);
        sensitive << Din;

    }

    virtual ~top_xlslice_0_0_core() = default;
